...
public class DhcpSubnetObject extends DhcpEntityObject {
	...
	public DhcpSubnetObject ( Linux_DHCPEntity fco, DhcpCimAdapter adapter ) throws WbemsmtException {
		...
		List<Linux_DHCPOptions> SubnetopArrayList = ((Linux_DHCPSubnet)fco).getAssociated_Linux_DHCPOptions_Linux_DHCPOptionsForEntitys (
						adapter.getCimClient ());

		for (Iterator<Linux_DHCPOptions> iter = SubnetopArrayList.iterator (); iter.hasNext ();) {
		...
		List<Linux_DHCPParams> SubnetparamArrayList = ((Linux_DHCPSubnet) fco).getAssociated_Linux_DHCPParams_Linux_DHCPParamsForEntitys (
				adapter.getCimClient () );

		for (Iterator<Linux_DHCPParams> iter = SubnetparamArrayList.iterator (); iter.hasNext ();) {
	...
	public MessageList save(DHCPSubnetOptionsContainer container) throws WbemsmtException{
		...
		boolean objFound = false;
			...
			for (Iterator<LabeledBaseInputComponentIf> iter2 = fields.iterator (); iter2.hasNext ();){
				...
				objFound = false;
				for (Iterator<Object> iter1 = getSubnetoptionslist ().iterator (); iter1.hasNext ();){
...
//					obj = new DhcpOptionsObject ( opFco, adapter );
	...
	public MessageList save(DHCPSubnetParamsContainer container) throws WbemsmtException{
		...
		boolean objFound = false;
			...
			for (Iterator<LabeledBaseInputComponentIf> iter2 = fields.iterator (); iter2.hasNext ();){
				LabeledBaseInputComponentIf fld = (LabeledBaseInputComponentIf)iter2.next();
				objFound = false;
				for (Iterator<Object> iter1 = getSubnetparamslist ().iterator (); iter1.hasNext ();){
	...
	public void updateControls(DHCPSubnetParamsContainer container){
		...
		for (Iterator<Object> iter = getSubnetparamslist ().iterator (); iter.hasNext ();){ 
	...
	public void updateControls(DHCPSubnetOptionsContainer container){
		...
		for (Iterator<Object> iter = getSubnetoptionslist ().iterator (); iter.hasNext ();){ 
...
//		super.updateControls ( container );
	...
	public void deleteSubnet () throws WbemsmtException {
		...
		List<CIMObjectPath> list = Linux_DHCPSubnetsForEntityHelper.enumerateInstanceNames ( adapter.getCimClient (),adapter.getNamespace (), true );
