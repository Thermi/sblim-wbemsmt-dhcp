...
public final class Linux_DHCPHost_IndHelper {

	public final static String CIM_CLASS_NAME = "Linux_DHCPHost_Ind";
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					if (!deep && cimInstance.getClassName().equals(CIM_CLASS_NAME)) {
						resultArrayList.add(new Linux_DHCPHost_Ind(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					Class clazz = Linux_DHCPHost_IndHelper.findClass(cimClient, cimInstance);
					if (clazz == null) {
	...
	*/	
	public final static Linux_DHCPHost_Ind getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return Linux_DHCPHost_IndHelper.getInstance(cimClient, new Linux_DHCPHost_Ind(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static Linux_DHCPHost_Ind getInstance(CIMClient cimClient, Linux_DHCPHost_Ind dataInstance) {
		...
		return Linux_DHCPHost_IndHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static Linux_DHCPHost_Ind getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		try {
			...
			{
				dataInstance = new Linux_DHCPHost_Ind(cimObjectPath, cimInstance);
			}
		} catch (Exception e) {
	...
	public static Class findClass(CIMClient cimClient, CIMInstance cimInstance) {
		...
		String[] packageList = Linux_DHCPHost_Ind.getPackages();
	...
	*/	
	public final static Linux_DHCPHost_Ind createInstance(CIMClient cimClient, Linux_DHCPHost_Ind dataInstance, boolean notifyDifferencies){ 
		...
		Linux_DHCPHost_Ind newDataInstance = null;
		try {
			newDataInstance = Linux_DHCPHost_IndHelper.getInstance(cimClient, cimObjectPath);
		} catch (Exception e) {
	...
	*/	
	public final static Linux_DHCPHost_Ind modifyInstance(CIMClient cimClient, Linux_DHCPHost_Ind dataInstance, boolean notifyDifferencies){ 
		...
		if (dataInstance.isModified()) {
			...
			dataInstance = Linux_DHCPHost_IndHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
		}
	...
	*/	
	public final static void deleteInstance(CIMClient cimClient, Linux_DHCPHost_Ind dataInstance){
	...
	*	no description
	*/
	private final static CIMInstance copyDataToServerInstance(CIMClient cimClient, Linux_DHCPHost_Ind dataInstance, boolean notifyDifferencies) {
		...
		for (int i = 0; i < serverPropertyListVector.size(); i++) {
			if ((CIMProperty)serverPropertyListVector.get(i) != null && 
					Linux_DHCPHost_Ind.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyListVector.get(i)).getName())) {
	...
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!Linux_DHCPHost_Ind.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < Linux_DHCPHost_Ind.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(Linux_DHCPHost_Ind.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(Linux_DHCPHost_Ind.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
