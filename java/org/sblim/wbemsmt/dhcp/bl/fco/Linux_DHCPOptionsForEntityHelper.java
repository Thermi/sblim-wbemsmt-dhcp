...
public final class Linux_DHCPOptionsForEntityHelper {
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					if (!deep && cimInstance.getClassName().equals(CIM_CLASS_NAME)) {
						resultArrayList.add(new Linux_DHCPOptionsForEntity(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
	...
	*/	
	public final static Linux_DHCPOptionsForEntity getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return Linux_DHCPOptionsForEntityHelper.getInstance(cimClient, new Linux_DHCPOptionsForEntity(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static Linux_DHCPOptionsForEntity getInstance(CIMClient cimClient, Linux_DHCPOptionsForEntity dataInstance) {
		...
		return Linux_DHCPOptionsForEntityHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static Linux_DHCPOptionsForEntity getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		try {
			...
			{
				dataInstance = new Linux_DHCPOptionsForEntity(cimObjectPath, cimInstance);
			}
		} catch (Exception e) {
	...
	public static Class findClass(CIMClient cimClient, CIMInstance cimInstance) {
		...
		String[] packageList = Linux_DHCPOptionsForEntity.getPackages();
	...
	*/	
	public final static Linux_DHCPOptionsForEntity createInstance(CIMClient cimClient, Linux_DHCPOptionsForEntity dataInstance, boolean notifyDifferencies){ 
		...
		Linux_DHCPOptionsForEntity newDataInstance = null;
		try {
			newDataInstance = Linux_DHCPOptionsForEntityHelper.getInstance(cimClient, cimObjectPath);
		} catch (Exception e) {
	...
	*/	
	public final static Linux_DHCPOptionsForEntity modifyInstance(CIMClient cimClient, Linux_DHCPOptionsForEntity dataInstance, boolean notifyDifferencies){ 
		...
		if (dataInstance.isModified()) {
			...
			dataInstance = Linux_DHCPOptionsForEntityHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
		}
	...
	*/	
	public final static void deleteInstance(CIMClient cimClient, Linux_DHCPOptionsForEntity dataInstance){
	...
	*	no description
	*/
	private final static CIMInstance copyDataToServerInstance(CIMClient cimClient, Linux_DHCPOptionsForEntity dataInstance, boolean notifyDifferencies) {
		...
		for (int i = 0; i < serverPropertyListVector.size(); i++) {
			if ((CIMProperty)serverPropertyListVector.get(i) != null && 
					Linux_DHCPOptionsForEntity.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyListVector.get(i)).getName())) {
	...
	*/	
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!Linux_DHCPOptionsForEntity.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < Linux_DHCPOptionsForEntity.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(Linux_DHCPOptionsForEntity.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(Linux_DHCPOptionsForEntity.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
	...
	public static boolean isValid_Linux_DHCPOptions(Linux_DHCPOptions Linux_DHCPOptions) {
		return true;
	}
