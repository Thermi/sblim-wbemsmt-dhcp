...
public final class Linux_DHCPSharednetHelper {
	...
	*/	
	public final static Linux_DHCPSharednet getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return Linux_DHCPSharednetHelper.getInstance(cimClient, new Linux_DHCPSharednet(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static Linux_DHCPSharednet getInstance(CIMClient cimClient, Linux_DHCPSharednet dataInstance) {
		...
		return Linux_DHCPSharednetHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static Linux_DHCPSharednet getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		try {
			...
			{
				dataInstance = new Linux_DHCPSharednet(cimObjectPath, cimInstance);
			}
		} catch (Exception e) {
	...
	public static Class findClass(CIMClient cimClient, CIMInstance cimInstance) {
		...
		String[] packageList = Linux_DHCPSharednet.getPackages();
	...
	*/	
	public final static Linux_DHCPSharednet createInstance(CIMClient cimClient, Linux_DHCPSharednet dataInstance, boolean notifyDifferencies){ 
		...
		Linux_DHCPSharednet newDataInstance = null;
		try {
			newDataInstance = Linux_DHCPSharednetHelper.getInstance(cimClient, cimObjectPath);
		} catch (Exception e) {
	...
	*/	
	public final static Linux_DHCPSharednet modifyInstance(CIMClient cimClient, Linux_DHCPSharednet dataInstance, boolean notifyDifferencies){ 
		...
		if (dataInstance.isModified()) {
			...
			dataInstance = Linux_DHCPSharednetHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
		}
	...
	*/	
	public final static void deleteInstance(CIMClient cimClient, Linux_DHCPSharednet dataInstance){
	...
	*	no description
	*/
	private final static CIMInstance copyDataToServerInstance(CIMClient cimClient, Linux_DHCPSharednet dataInstance, boolean notifyDifferencies) {
		...
		for (int i = 0; i < serverPropertyListVector.size(); i++) {
			if ((CIMProperty)serverPropertyListVector.get(i) != null && 
					Linux_DHCPSharednet.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyListVector.get(i)).getName())) {
	...
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!Linux_DHCPSharednet.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < Linux_DHCPSharednet.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(Linux_DHCPSharednet.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(Linux_DHCPSharednet.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
