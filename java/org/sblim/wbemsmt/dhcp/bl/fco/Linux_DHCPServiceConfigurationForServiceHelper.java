...
public final class Linux_DHCPServiceConfigurationForServiceHelper {

	public final static String CIM_CLASS_NAME = "Linux_DHCPServiceConfigurationForService";
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					if (!deep && cimInstance.getClassName().equals(CIM_CLASS_NAME)) {
						resultArrayList.add(new Linux_DHCPServiceConfigurationForService(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
	...
	*/	
	public final static Linux_DHCPServiceConfigurationForService getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return Linux_DHCPServiceConfigurationForServiceHelper.getInstance(cimClient, new Linux_DHCPServiceConfigurationForService(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static Linux_DHCPServiceConfigurationForService getInstance(CIMClient cimClient, Linux_DHCPServiceConfigurationForService dataInstance) {
		...
		return Linux_DHCPServiceConfigurationForServiceHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static Linux_DHCPServiceConfigurationForService getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		try {
			...
			{
				dataInstance = new Linux_DHCPServiceConfigurationForService(cimObjectPath, cimInstance);
			}
		} catch (Exception e) {
	...
	public static Class findClass(CIMClient cimClient, CIMInstance cimInstance) {
		...
		String[] packageList = Linux_DHCPServiceConfigurationForService.getPackages();
	...
	*/	
	public final static Linux_DHCPServiceConfigurationForService createInstance(CIMClient cimClient, Linux_DHCPServiceConfigurationForService dataInstance, boolean notifyDifferencies){ 
		...
		Linux_DHCPServiceConfigurationForService newDataInstance = null;
		try {
			newDataInstance = Linux_DHCPServiceConfigurationForServiceHelper.getInstance(cimClient, cimObjectPath);
		} catch (Exception e) {
	...
	*/	
	public final static Linux_DHCPServiceConfigurationForService modifyInstance(CIMClient cimClient, Linux_DHCPServiceConfigurationForService dataInstance, boolean notifyDifferencies){ 
		...
		if (dataInstance.isModified()) {
			...
			dataInstance = Linux_DHCPServiceConfigurationForServiceHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
		}
	...
	*/	
	public final static void deleteInstance(CIMClient cimClient, Linux_DHCPServiceConfigurationForService dataInstance){
	...
	*	no description
	*/
	private final static CIMInstance copyDataToServerInstance(CIMClient cimClient, Linux_DHCPServiceConfigurationForService dataInstance, boolean notifyDifferencies) {
		...
		for (int i = 0; i < serverPropertyListVector.size(); i++) {
			if ((CIMProperty)serverPropertyListVector.get(i) != null && 
					Linux_DHCPServiceConfigurationForService.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyListVector.get(i)).getName())) {
	...
	*/	
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!Linux_DHCPServiceConfigurationForService.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < Linux_DHCPServiceConfigurationForService.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(Linux_DHCPServiceConfigurationForService.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(Linux_DHCPServiceConfigurationForService.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
	...
	public static boolean isValid_Linux_DHCPService(Linux_DHCPService Linux_DHCPService) {
		return true;
	}

	public static boolean isValid_Linux_DHCPServiceConfiguration(Linux_DHCPServiceConfiguration Linux_DHCPServiceConfiguration) {
		return true;
	}
