...
public final class Linux_DHCPParamsHelper {
	...
	*/	
	public final static Linux_DHCPParams getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return Linux_DHCPParamsHelper.getInstance(cimClient, new Linux_DHCPParams(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static Linux_DHCPParams getInstance(CIMClient cimClient, Linux_DHCPParams dataInstance) {
		...
		return Linux_DHCPParamsHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static Linux_DHCPParams getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		try {
			...
			{
				dataInstance = new Linux_DHCPParams(cimObjectPath, cimInstance);
			}
		} catch (Exception e) {
	...
	public static Class findClass(CIMClient cimClient, CIMInstance cimInstance) {
		...
		String[] packageList = Linux_DHCPParams.getPackages();
	...
	*/	
	public final static Linux_DHCPParams createInstance(CIMClient cimClient, Linux_DHCPParams dataInstance, boolean notifyDifferencies){ 
		...
		Linux_DHCPParams newDataInstance = null;
		try {
			newDataInstance = Linux_DHCPParamsHelper.getInstance(cimClient, cimObjectPath);
		} catch (Exception e) {
	...
	*/	
	public final static Linux_DHCPParams modifyInstance(CIMClient cimClient, Linux_DHCPParams dataInstance, boolean notifyDifferencies){ 
		...
		if (dataInstance.isModified()) {
			...
			dataInstance = Linux_DHCPParamsHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
		}
	...
	*/	
	public final static void deleteInstance(CIMClient cimClient, Linux_DHCPParams dataInstance){
	...
	*	no description
	*/
	private final static CIMInstance copyDataToServerInstance(CIMClient cimClient, Linux_DHCPParams dataInstance, boolean notifyDifferencies) {
		...
		for (int i = 0; i < serverPropertyListVector.size(); i++) {
			if ((CIMProperty)serverPropertyListVector.get(i) != null && 
					Linux_DHCPParams.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyListVector.get(i)).getName())) {
	...
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!Linux_DHCPParams.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < Linux_DHCPParams.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(Linux_DHCPParams.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(Linux_DHCPParams.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
