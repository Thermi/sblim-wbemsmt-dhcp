...
public final class Linux_DHCPSubnetHelper {
	...
	*/	
	public final static Linux_DHCPSubnet getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return Linux_DHCPSubnetHelper.getInstance(cimClient, new Linux_DHCPSubnet(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static Linux_DHCPSubnet getInstance(CIMClient cimClient, Linux_DHCPSubnet dataInstance) {
		...
		return Linux_DHCPSubnetHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static Linux_DHCPSubnet getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		try {
			...
			{
				dataInstance = new Linux_DHCPSubnet(cimObjectPath, cimInstance);
			}
		} catch (Exception e) {
	...
	public static Class findClass(CIMClient cimClient, CIMInstance cimInstance) {
		...
		String[] packageList = Linux_DHCPSubnet.getPackages();
	...
	*/	
	public final static Linux_DHCPSubnet createInstance(CIMClient cimClient, Linux_DHCPSubnet dataInstance, boolean notifyDifferencies){ 
		...
		Linux_DHCPSubnet newDataInstance = null;
		try {
			newDataInstance = Linux_DHCPSubnetHelper.getInstance(cimClient, cimObjectPath);
		} catch (Exception e) {
	...
	*/	
	public final static Linux_DHCPSubnet modifyInstance(CIMClient cimClient, Linux_DHCPSubnet dataInstance, boolean notifyDifferencies){ 
		...
		if (dataInstance.isModified()) {
			...
			dataInstance = Linux_DHCPSubnetHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
		}
	...
	*/	
	public final static void deleteInstance(CIMClient cimClient, Linux_DHCPSubnet dataInstance){
	...
	*	no description
	*/
	private final static CIMInstance copyDataToServerInstance(CIMClient cimClient, Linux_DHCPSubnet dataInstance, boolean notifyDifferencies) {
		...
		for (int i = 0; i < serverPropertyListVector.size(); i++) {
			if ((CIMProperty)serverPropertyListVector.get(i) != null && 
					Linux_DHCPSubnet.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyListVector.get(i)).getName())) {
	...
	*/	
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!Linux_DHCPSubnet.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < Linux_DHCPSubnet.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(Linux_DHCPSubnet.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(Linux_DHCPSubnet.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
	...
	public static boolean isValid_Netmask(String Netmask) {
		return true;
	}
