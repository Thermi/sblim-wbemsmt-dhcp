...
public final class Linux_DHCPGroupHelper {
	...
	*/	
	public final static Linux_DHCPGroup getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return Linux_DHCPGroupHelper.getInstance(cimClient, new Linux_DHCPGroup(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static Linux_DHCPGroup getInstance(CIMClient cimClient, Linux_DHCPGroup dataInstance) {
		...
		return Linux_DHCPGroupHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static Linux_DHCPGroup getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		try {
			...
			{
				dataInstance = new Linux_DHCPGroup(cimObjectPath, cimInstance);
			}
		} catch (Exception e) {
	...
	public static Class findClass(CIMClient cimClient, CIMInstance cimInstance) {
		...
		String[] packageList = Linux_DHCPGroup.getPackages();
	...
	*/	
	public final static Linux_DHCPGroup createInstance(CIMClient cimClient, Linux_DHCPGroup dataInstance, boolean notifyDifferencies){ 
		...
		Linux_DHCPGroup newDataInstance = null;
		try {
			newDataInstance = Linux_DHCPGroupHelper.getInstance(cimClient, cimObjectPath);
		} catch (Exception e) {
	...
	*/	
	public final static Linux_DHCPGroup modifyInstance(CIMClient cimClient, Linux_DHCPGroup dataInstance, boolean notifyDifferencies){ 
		...
		if (dataInstance.isModified()) {
			...
			dataInstance = Linux_DHCPGroupHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
		}
	...
	*/	
	public final static void deleteInstance(CIMClient cimClient, Linux_DHCPGroup dataInstance){
	...
	*	no description
	*/
	private final static CIMInstance copyDataToServerInstance(CIMClient cimClient, Linux_DHCPGroup dataInstance, boolean notifyDifferencies) {
		...
		for (int i = 0; i < serverPropertyListVector.size(); i++) {
			if ((CIMProperty)serverPropertyListVector.get(i) != null && 
					Linux_DHCPGroup.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyListVector.get(i)).getName())) {
	...
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!Linux_DHCPGroup.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < Linux_DHCPGroup.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(Linux_DHCPGroup.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(Linux_DHCPGroup.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
