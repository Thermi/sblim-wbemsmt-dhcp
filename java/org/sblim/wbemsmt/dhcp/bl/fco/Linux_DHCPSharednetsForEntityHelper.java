...
public final class Linux_DHCPSharednetsForEntityHelper {
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					if (!deep && cimInstance.getClassName().equals(CIM_CLASS_NAME)) {
						resultArrayList.add(new Linux_DHCPSharednetsForEntity(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
	...
	*/	
	public final static Linux_DHCPSharednetsForEntity getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return Linux_DHCPSharednetsForEntityHelper.getInstance(cimClient, new Linux_DHCPSharednetsForEntity(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static Linux_DHCPSharednetsForEntity getInstance(CIMClient cimClient, Linux_DHCPSharednetsForEntity dataInstance) {
		...
		return Linux_DHCPSharednetsForEntityHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static Linux_DHCPSharednetsForEntity getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		try {
			...
			{
				dataInstance = new Linux_DHCPSharednetsForEntity(cimObjectPath, cimInstance);
			}
		} catch (Exception e) {
	...
	public static Class findClass(CIMClient cimClient, CIMInstance cimInstance) {
		...
		String[] packageList = Linux_DHCPSharednetsForEntity.getPackages();
	...
	*/	
	public final static Linux_DHCPSharednetsForEntity createInstance(CIMClient cimClient, Linux_DHCPSharednetsForEntity dataInstance, boolean notifyDifferencies){ 
		...
		Linux_DHCPSharednetsForEntity newDataInstance = null;
		try {
			newDataInstance = Linux_DHCPSharednetsForEntityHelper.getInstance(cimClient, cimObjectPath);
		} catch (Exception e) {
	...
	*/	
	public final static Linux_DHCPSharednetsForEntity modifyInstance(CIMClient cimClient, Linux_DHCPSharednetsForEntity dataInstance, boolean notifyDifferencies){ 
		...
		if (dataInstance.isModified()) {
			...
			dataInstance = Linux_DHCPSharednetsForEntityHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
		}
	...
	*/	
	public final static void deleteInstance(CIMClient cimClient, Linux_DHCPSharednetsForEntity dataInstance){
	...
	*	no description
	*/
	private final static CIMInstance copyDataToServerInstance(CIMClient cimClient, Linux_DHCPSharednetsForEntity dataInstance, boolean notifyDifferencies) {
		...
		for (int i = 0; i < serverPropertyListVector.size(); i++) {
			if ((CIMProperty)serverPropertyListVector.get(i) != null && 
					Linux_DHCPSharednetsForEntity.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyListVector.get(i)).getName())) {
	...
	*/	
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!Linux_DHCPSharednetsForEntity.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < Linux_DHCPSharednetsForEntity.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(Linux_DHCPSharednetsForEntity.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(Linux_DHCPSharednetsForEntity.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
	...
	public static boolean isValid_Linux_DHCPEntity(Linux_DHCPEntity Linux_DHCPEntity) {
		return true;
	}

	public static boolean isValid_Linux_DHCPSharednet(Linux_DHCPSharednet Linux_DHCPSharednet) {
		return true;
	}
