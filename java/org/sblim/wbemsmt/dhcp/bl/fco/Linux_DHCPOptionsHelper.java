...
public final class Linux_DHCPOptionsHelper {
	...
	*/	
	public final static Linux_DHCPOptions getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return Linux_DHCPOptionsHelper.getInstance(cimClient, new Linux_DHCPOptions(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static Linux_DHCPOptions getInstance(CIMClient cimClient, Linux_DHCPOptions dataInstance) {
		...
		return Linux_DHCPOptionsHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static Linux_DHCPOptions getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		try {
			...
			{
				dataInstance = new Linux_DHCPOptions(cimObjectPath, cimInstance);
			}
		} catch (Exception e) {
	...
	public static Class findClass(CIMClient cimClient, CIMInstance cimInstance) {
		...
		String[] packageList = Linux_DHCPOptions.getPackages();
	...
	*/	
	public final static Linux_DHCPOptions createInstance(CIMClient cimClient, Linux_DHCPOptions dataInstance, boolean notifyDifferencies){ 
		...
		Linux_DHCPOptions newDataInstance = null;
		try {
			newDataInstance = Linux_DHCPOptionsHelper.getInstance(cimClient, cimObjectPath);
		} catch (Exception e) {
	...
	*/	
	public final static Linux_DHCPOptions modifyInstance(CIMClient cimClient, Linux_DHCPOptions dataInstance, boolean notifyDifferencies){ 
		...
		if (dataInstance.isModified()) {
			...
			dataInstance = Linux_DHCPOptionsHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
		}
	...
	*/	
	public final static void deleteInstance(CIMClient cimClient, Linux_DHCPOptions dataInstance){
	...
	*	no description
	*/
	private final static CIMInstance copyDataToServerInstance(CIMClient cimClient, Linux_DHCPOptions dataInstance, boolean notifyDifferencies) {
		...
		for (int i = 0; i < serverPropertyListVector.size(); i++) {
			if ((CIMProperty)serverPropertyListVector.get(i) != null && 
					Linux_DHCPOptions.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyListVector.get(i)).getName())) {
	...
	*/	
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!Linux_DHCPOptions.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < Linux_DHCPOptions.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(Linux_DHCPOptions.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(Linux_DHCPOptions.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
	...
	public static boolean isValid_ParentID(String ParentID) {
		return true;
	}

	public static boolean isValid_Values(String Values) {
		return true;
	}
