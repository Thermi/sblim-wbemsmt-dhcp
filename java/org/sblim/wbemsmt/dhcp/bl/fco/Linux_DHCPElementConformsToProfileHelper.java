...
public final class Linux_DHCPElementConformsToProfileHelper {
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					if (!deep && cimInstance.getClassName().equals(CIM_CLASS_NAME)) {
						resultArrayList.add(new Linux_DHCPElementConformsToProfile(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
					Class clazz = Linux_DHCPElementConformsToProfileHelper.findClass(cimClient, cimInstance);
					if (clazz == null) {
	...
	*/	
	public final static Linux_DHCPElementConformsToProfile getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return Linux_DHCPElementConformsToProfileHelper.getInstance(cimClient, new Linux_DHCPElementConformsToProfile(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static Linux_DHCPElementConformsToProfile getInstance(CIMClient cimClient, Linux_DHCPElementConformsToProfile dataInstance) {
		...
		return Linux_DHCPElementConformsToProfileHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static Linux_DHCPElementConformsToProfile getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		try {
			...
			{
				dataInstance = new Linux_DHCPElementConformsToProfile(cimObjectPath, cimInstance);
			}
		} catch (Exception e) {
	...
	public static Class findClass(CIMClient cimClient, CIMInstance cimInstance) {
		...
		String[] packageList = Linux_DHCPElementConformsToProfile.getPackages();
	...
	*/	
	public final static Linux_DHCPElementConformsToProfile createInstance(CIMClient cimClient, Linux_DHCPElementConformsToProfile dataInstance, boolean notifyDifferencies){ 
		...
		Linux_DHCPElementConformsToProfile newDataInstance = null;
		try {
			newDataInstance = Linux_DHCPElementConformsToProfileHelper.getInstance(cimClient, cimObjectPath);
		} catch (Exception e) {
	...
	*/	
	public final static Linux_DHCPElementConformsToProfile modifyInstance(CIMClient cimClient, Linux_DHCPElementConformsToProfile dataInstance, boolean notifyDifferencies){ 
		...
		if (dataInstance.isModified()) {
			...
			dataInstance = Linux_DHCPElementConformsToProfileHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
		}
	...
	*/	
	public final static void deleteInstance(CIMClient cimClient, Linux_DHCPElementConformsToProfile dataInstance){
	...
	*	no description
	*/
	private final static CIMInstance copyDataToServerInstance(CIMClient cimClient, Linux_DHCPElementConformsToProfile dataInstance, boolean notifyDifferencies) {
		...
		for (int i = 0; i < serverPropertyListVector.size(); i++) {
			if ((CIMProperty)serverPropertyListVector.get(i) != null && 
					Linux_DHCPElementConformsToProfile.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyListVector.get(i)).getName())) {
	...
	*/	
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!Linux_DHCPElementConformsToProfile.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < Linux_DHCPElementConformsToProfile.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(Linux_DHCPElementConformsToProfile.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(Linux_DHCPElementConformsToProfile.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
	...
	public static boolean isValid_Linux_DHCPRegisteredProfile(Linux_DHCPRegisteredProfile Linux_DHCPRegisteredProfile) {
		return true;
	}
