...
public final class Linux_DHCPPoolHelper {
	...
	*/	
	public final static Linux_DHCPPool getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return Linux_DHCPPoolHelper.getInstance(cimClient, new Linux_DHCPPool(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static Linux_DHCPPool getInstance(CIMClient cimClient, Linux_DHCPPool dataInstance) {
		...
		return Linux_DHCPPoolHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static Linux_DHCPPool getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		try {
			...
			{
				dataInstance = new Linux_DHCPPool(cimObjectPath, cimInstance);
			}
		} catch (Exception e) {
	...
	public static Class findClass(CIMClient cimClient, CIMInstance cimInstance) {
		...
		String[] packageList = Linux_DHCPPool.getPackages();
	...
	*/	
	public final static Linux_DHCPPool createInstance(CIMClient cimClient, Linux_DHCPPool dataInstance, boolean notifyDifferencies){ 
		...
		Linux_DHCPPool newDataInstance = null;
		try {
			newDataInstance = Linux_DHCPPoolHelper.getInstance(cimClient, cimObjectPath);
		} catch (Exception e) {
	...
	*/	
	public final static Linux_DHCPPool modifyInstance(CIMClient cimClient, Linux_DHCPPool dataInstance, boolean notifyDifferencies){ 
		...
		if (dataInstance.isModified()) {
			...
			dataInstance = Linux_DHCPPoolHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
		}
	...
	*/	
	public final static void deleteInstance(CIMClient cimClient, Linux_DHCPPool dataInstance){
	...
	*	no description
	*/
	private final static CIMInstance copyDataToServerInstance(CIMClient cimClient, Linux_DHCPPool dataInstance, boolean notifyDifferencies) {
		...
		for (int i = 0; i < serverPropertyListVector.size(); i++) {
			if ((CIMProperty)serverPropertyListVector.get(i) != null && 
					Linux_DHCPPool.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyListVector.get(i)).getName())) {
	...
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!Linux_DHCPPool.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < Linux_DHCPPool.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(Linux_DHCPPool.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(Linux_DHCPPool.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
