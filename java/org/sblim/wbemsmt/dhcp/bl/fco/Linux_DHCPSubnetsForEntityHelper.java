...
public final class Linux_DHCPSubnetsForEntityHelper {

	public final static String CIM_CLASS_NAME = "Linux_DHCPSubnetsForEntity";
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					if (!deep && cimInstance.getClassName().equals(CIM_CLASS_NAME)) {
						resultArrayList.add(new Linux_DHCPSubnetsForEntity(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
	...
	*/	
	public final static Linux_DHCPSubnetsForEntity getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return Linux_DHCPSubnetsForEntityHelper.getInstance(cimClient, new Linux_DHCPSubnetsForEntity(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static Linux_DHCPSubnetsForEntity getInstance(CIMClient cimClient, Linux_DHCPSubnetsForEntity dataInstance) {
		...
		return Linux_DHCPSubnetsForEntityHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static Linux_DHCPSubnetsForEntity getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		try {
			...
			{
				dataInstance = new Linux_DHCPSubnetsForEntity(cimObjectPath, cimInstance);
			}
		} catch (Exception e) {
	...
	public static Class findClass(CIMClient cimClient, CIMInstance cimInstance) {
		...
		String[] packageList = Linux_DHCPSubnetsForEntity.getPackages();
	...
	*/	
	public final static Linux_DHCPSubnetsForEntity createInstance(CIMClient cimClient, Linux_DHCPSubnetsForEntity dataInstance, boolean notifyDifferencies){ 
		...
		Linux_DHCPSubnetsForEntity newDataInstance = null;
		try {
			newDataInstance = Linux_DHCPSubnetsForEntityHelper.getInstance(cimClient, cimObjectPath);
		} catch (Exception e) {
	...
	*/	
	public final static Linux_DHCPSubnetsForEntity modifyInstance(CIMClient cimClient, Linux_DHCPSubnetsForEntity dataInstance, boolean notifyDifferencies){ 
		...
		if (dataInstance.isModified()) {
			...
			dataInstance = Linux_DHCPSubnetsForEntityHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
		}
	...
	*/	
	public final static void deleteInstance(CIMClient cimClient, Linux_DHCPSubnetsForEntity dataInstance){
	...
	*	no description
	*/
	private final static CIMInstance copyDataToServerInstance(CIMClient cimClient, Linux_DHCPSubnetsForEntity dataInstance, boolean notifyDifferencies) {
		...
		for (int i = 0; i < serverPropertyListVector.size(); i++) {
			if ((CIMProperty)serverPropertyListVector.get(i) != null && 
					Linux_DHCPSubnetsForEntity.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyListVector.get(i)).getName())) {
	...
	*/	
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!Linux_DHCPSubnetsForEntity.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < Linux_DHCPSubnetsForEntity.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(Linux_DHCPSubnetsForEntity.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(Linux_DHCPSubnetsForEntity.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
	...
	public static boolean isValid_Linux_DHCPSubnet(Linux_DHCPSubnet Linux_DHCPSubnet) {
		return true;
	}
