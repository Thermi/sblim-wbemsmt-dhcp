...
public final class Linux_DHCPServiceHelper {
	...
	*/	
	public final static Linux_DHCPService getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return Linux_DHCPServiceHelper.getInstance(cimClient, new Linux_DHCPService(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static Linux_DHCPService getInstance(CIMClient cimClient, Linux_DHCPService dataInstance) {
		...
		return Linux_DHCPServiceHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static Linux_DHCPService getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		try {
			...
			{
				dataInstance = new Linux_DHCPService(cimObjectPath, cimInstance);
			}
		} catch (Exception e) {
	...
	public static Class findClass(CIMClient cimClient, CIMInstance cimInstance) {
		...
		String[] packageList = Linux_DHCPService.getPackages();
	...
	*/	
	public final static Linux_DHCPService createInstance(CIMClient cimClient, Linux_DHCPService dataInstance, boolean notifyDifferencies){ 
		...
		Linux_DHCPService newDataInstance = null;
		try {
			newDataInstance = Linux_DHCPServiceHelper.getInstance(cimClient, cimObjectPath);
		} catch (Exception e) {
	...
	*/	
	public final static Linux_DHCPService modifyInstance(CIMClient cimClient, Linux_DHCPService dataInstance, boolean notifyDifferencies){ 
		...
		if (dataInstance.isModified()) {
			...
			dataInstance = Linux_DHCPServiceHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
		}
	...
	*/	
	public final static void deleteInstance(CIMClient cimClient, Linux_DHCPService dataInstance){
	...
	*	no description
	*/
	private final static CIMInstance copyDataToServerInstance(CIMClient cimClient, Linux_DHCPService dataInstance, boolean notifyDifferencies) {
		...
		for (int i = 0; i < serverPropertyListVector.size(); i++) {
			if ((CIMProperty)serverPropertyListVector.get(i) != null && 
					Linux_DHCPService.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyListVector.get(i)).getName())) {
	...
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!Linux_DHCPService.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < Linux_DHCPService.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(Linux_DHCPService.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(Linux_DHCPService.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
