...
public final class Linux_DHCPGlobalHelper {
	...
	*/	
	public final static Linux_DHCPGlobal getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return Linux_DHCPGlobalHelper.getInstance(cimClient, new Linux_DHCPGlobal(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static Linux_DHCPGlobal getInstance(CIMClient cimClient, Linux_DHCPGlobal dataInstance) {
		...
		return Linux_DHCPGlobalHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static Linux_DHCPGlobal getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		try {
			...
			{
				dataInstance = new Linux_DHCPGlobal(cimObjectPath, cimInstance);
			}
		} catch (Exception e) {
	...
	public static Class findClass(CIMClient cimClient, CIMInstance cimInstance) {
		...
		String[] packageList = Linux_DHCPGlobal.getPackages();
	...
	*/	
	public final static Linux_DHCPGlobal createInstance(CIMClient cimClient, Linux_DHCPGlobal dataInstance, boolean notifyDifferencies){ 
		...
		Linux_DHCPGlobal newDataInstance = null;
		try {
			newDataInstance = Linux_DHCPGlobalHelper.getInstance(cimClient, cimObjectPath);
		} catch (Exception e) {
	...
	*/	
	public final static Linux_DHCPGlobal modifyInstance(CIMClient cimClient, Linux_DHCPGlobal dataInstance, boolean notifyDifferencies){ 
		...
		if (dataInstance.isModified()) {
			...
			dataInstance = Linux_DHCPGlobalHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
		}
	...
	*/	
	public final static void deleteInstance(CIMClient cimClient, Linux_DHCPGlobal dataInstance){
	...
	*	no description
	*/
	private final static CIMInstance copyDataToServerInstance(CIMClient cimClient, Linux_DHCPGlobal dataInstance, boolean notifyDifferencies) {
		...
		for (int i = 0; i < serverPropertyListVector.size(); i++) {
			if ((CIMProperty)serverPropertyListVector.get(i) != null && 
					Linux_DHCPGlobal.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyListVector.get(i)).getName())) {
	...
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!Linux_DHCPGlobal.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < Linux_DHCPGlobal.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(Linux_DHCPGlobal.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(Linux_DHCPGlobal.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
