...
public final class Linux_DHCPParamsForEntityHelper {
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					if (!deep && cimInstance.getClassName().equals(CIM_CLASS_NAME)) {
						resultArrayList.add(new Linux_DHCPParamsForEntity(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
	...
	*/	
	public final static Linux_DHCPParamsForEntity getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return Linux_DHCPParamsForEntityHelper.getInstance(cimClient, new Linux_DHCPParamsForEntity(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static Linux_DHCPParamsForEntity getInstance(CIMClient cimClient, Linux_DHCPParamsForEntity dataInstance) {
		...
		return Linux_DHCPParamsForEntityHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static Linux_DHCPParamsForEntity getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		try {
			...
			{
				dataInstance = new Linux_DHCPParamsForEntity(cimObjectPath, cimInstance);
			}
		} catch (Exception e) {
	...
	public static Class findClass(CIMClient cimClient, CIMInstance cimInstance) {
		...
		String[] packageList = Linux_DHCPParamsForEntity.getPackages();
	...
	*/	
	public final static Linux_DHCPParamsForEntity createInstance(CIMClient cimClient, Linux_DHCPParamsForEntity dataInstance, boolean notifyDifferencies){ 
		...
		Linux_DHCPParamsForEntity newDataInstance = null;
		try {
			newDataInstance = Linux_DHCPParamsForEntityHelper.getInstance(cimClient, cimObjectPath);
		} catch (Exception e) {
	...
	*/	
	public final static Linux_DHCPParamsForEntity modifyInstance(CIMClient cimClient, Linux_DHCPParamsForEntity dataInstance, boolean notifyDifferencies){ 
		...
		if (dataInstance.isModified()) {
			...
			dataInstance = Linux_DHCPParamsForEntityHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
		}
	...
	*/	
	public final static void deleteInstance(CIMClient cimClient, Linux_DHCPParamsForEntity dataInstance){
	...
	*	no description
	*/
	private final static CIMInstance copyDataToServerInstance(CIMClient cimClient, Linux_DHCPParamsForEntity dataInstance, boolean notifyDifferencies) {
		...
		for (int i = 0; i < serverPropertyListVector.size(); i++) {
			if ((CIMProperty)serverPropertyListVector.get(i) != null && 
					Linux_DHCPParamsForEntity.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyListVector.get(i)).getName())) {
	...
	*/	
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!Linux_DHCPParamsForEntity.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < Linux_DHCPParamsForEntity.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(Linux_DHCPParamsForEntity.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(Linux_DHCPParamsForEntity.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
	...
	public static boolean isValid_Linux_DHCPParams(Linux_DHCPParams Linux_DHCPParams) {
		return true;
	}
