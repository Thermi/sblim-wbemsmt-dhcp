...
public final class Linux_DHCPGlobalForServiceHelper {

	public final static String CIM_CLASS_NAME = "Linux_DHCPGlobalForService";
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					if (!deep && cimInstance.getClassName().equals(CIM_CLASS_NAME)) {
						resultArrayList.add(new Linux_DHCPGlobalForService(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
	...
	*/	
	public final static Linux_DHCPGlobalForService getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return Linux_DHCPGlobalForServiceHelper.getInstance(cimClient, new Linux_DHCPGlobalForService(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static Linux_DHCPGlobalForService getInstance(CIMClient cimClient, Linux_DHCPGlobalForService dataInstance) {
		...
		return Linux_DHCPGlobalForServiceHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static Linux_DHCPGlobalForService getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		try {
			...
			{
				dataInstance = new Linux_DHCPGlobalForService(cimObjectPath, cimInstance);
			}
		} catch (Exception e) {
	...
	public static Class findClass(CIMClient cimClient, CIMInstance cimInstance) {
		...
		String[] packageList = Linux_DHCPGlobalForService.getPackages();
	...
	*/	
	public final static Linux_DHCPGlobalForService createInstance(CIMClient cimClient, Linux_DHCPGlobalForService dataInstance, boolean notifyDifferencies){ 
		...
		Linux_DHCPGlobalForService newDataInstance = null;
		try {
			newDataInstance = Linux_DHCPGlobalForServiceHelper.getInstance(cimClient, cimObjectPath);
		} catch (Exception e) {
	...
	*/	
	public final static Linux_DHCPGlobalForService modifyInstance(CIMClient cimClient, Linux_DHCPGlobalForService dataInstance, boolean notifyDifferencies){ 
		...
		if (dataInstance.isModified()) {
			...
			dataInstance = Linux_DHCPGlobalForServiceHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
		}
	...
	*/	
	public final static void deleteInstance(CIMClient cimClient, Linux_DHCPGlobalForService dataInstance){
	...
	*	no description
	*/
	private final static CIMInstance copyDataToServerInstance(CIMClient cimClient, Linux_DHCPGlobalForService dataInstance, boolean notifyDifferencies) {
		...
		for (int i = 0; i < serverPropertyListVector.size(); i++) {
			if ((CIMProperty)serverPropertyListVector.get(i) != null && 
					Linux_DHCPGlobalForService.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyListVector.get(i)).getName())) {
	...
	*/	
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!Linux_DHCPGlobalForService.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < Linux_DHCPGlobalForService.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(Linux_DHCPGlobalForService.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(Linux_DHCPGlobalForService.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
	...
	public static boolean isValid_Linux_DHCPGlobal(Linux_DHCPGlobal Linux_DHCPGlobal) {
		return true;
	}
