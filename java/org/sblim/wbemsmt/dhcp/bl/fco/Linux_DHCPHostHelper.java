...
public final class Linux_DHCPHostHelper {
	...
	*/	
	public final static Linux_DHCPHost getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return Linux_DHCPHostHelper.getInstance(cimClient, new Linux_DHCPHost(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static Linux_DHCPHost getInstance(CIMClient cimClient, Linux_DHCPHost dataInstance) {
		...
		return Linux_DHCPHostHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static Linux_DHCPHost getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
	...
	public static Class findClass(CIMClient cimClient, CIMInstance cimInstance) {
		...
		String[] packageList = Linux_DHCPHost.getPackages();
	...
	*/	
	public final static Linux_DHCPHost createInstance(CIMClient cimClient, Linux_DHCPHost dataInstance, boolean notifyDifferencies){ 
		...
		Linux_DHCPHost newDataInstance = null;
		try {
			newDataInstance = Linux_DHCPHostHelper.getInstance(cimClient, cimObjectPath);
		} catch (Exception e) {
	...
	*/	
	public final static Linux_DHCPHost modifyInstance(CIMClient cimClient, Linux_DHCPHost dataInstance, boolean notifyDifferencies){ 
		...
		if (dataInstance.isModified()) {
			...
			dataInstance = Linux_DHCPHostHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
		}
	...
	*/	
	public final static void deleteInstance(CIMClient cimClient, Linux_DHCPHost dataInstance){
	...
	*	no description
	*/
	private final static CIMInstance copyDataToServerInstance(CIMClient cimClient, Linux_DHCPHost dataInstance, boolean notifyDifferencies) {
		...
		for (int i = 0; i < serverPropertyListVector.size(); i++) {
			if ((CIMProperty)serverPropertyListVector.get(i) != null && 
					Linux_DHCPHost.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyListVector.get(i)).getName())) {
	...
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!Linux_DHCPHost.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < Linux_DHCPHost.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(Linux_DHCPHost.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(Linux_DHCPHost.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
