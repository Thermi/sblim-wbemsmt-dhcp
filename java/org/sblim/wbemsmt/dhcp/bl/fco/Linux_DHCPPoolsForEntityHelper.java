...
public final class Linux_DHCPPoolsForEntityHelper {

	public final static String CIM_CLASS_NAME = "Linux_DHCPPoolsForEntity";
			...
			while (enumeration.hasMoreElements()) {
				...
				if (obj instanceof CIMInstance) {
					...
					if (!deep && cimInstance.getClassName().equals(CIM_CLASS_NAME)) {
						resultArrayList.add(new Linux_DHCPPoolsForEntity(cimInstance.getObjectPath(), cimInstance));
						continue;
					}
	...
	*/	
	public final static Linux_DHCPPoolsForEntity getInstance(CIMClient cimClient, Vector keyProperties) {
		...
		return Linux_DHCPPoolsForEntityHelper.getInstance(cimClient, new Linux_DHCPPoolsForEntity(keyProperties));
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static Linux_DHCPPoolsForEntity getInstance(CIMClient cimClient, Linux_DHCPPoolsForEntity dataInstance) {
		...
		return Linux_DHCPPoolsForEntityHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
	}
	...
	*	The method returns an instance of a given CIM client at a given path
	*/	
	public final static Linux_DHCPPoolsForEntity getInstance(CIMClient cimClient, CIMObjectPath cimObjectPath) {
		...
		try {
			...
			{
				dataInstance = new Linux_DHCPPoolsForEntity(cimObjectPath, cimInstance);
			}
		} catch (Exception e) {
	...
	public static Class findClass(CIMClient cimClient, CIMInstance cimInstance) {
		...
		String[] packageList = Linux_DHCPPoolsForEntity.getPackages();
	...
	*/	
	public final static Linux_DHCPPoolsForEntity createInstance(CIMClient cimClient, Linux_DHCPPoolsForEntity dataInstance, boolean notifyDifferencies){ 
		...
		Linux_DHCPPoolsForEntity newDataInstance = null;
		try {
			newDataInstance = Linux_DHCPPoolsForEntityHelper.getInstance(cimClient, cimObjectPath);
		} catch (Exception e) {
	...
	*/	
	public final static Linux_DHCPPoolsForEntity modifyInstance(CIMClient cimClient, Linux_DHCPPoolsForEntity dataInstance, boolean notifyDifferencies){ 
		...
		if (dataInstance.isModified()) {
			...
			dataInstance = Linux_DHCPPoolsForEntityHelper.getInstance(cimClient, dataInstance.getCimObjectPath());
		}
	...
	*/	
	public final static void deleteInstance(CIMClient cimClient, Linux_DHCPPoolsForEntity dataInstance){
	...
	*	no description
	*/
	private final static CIMInstance copyDataToServerInstance(CIMClient cimClient, Linux_DHCPPoolsForEntity dataInstance, boolean notifyDifferencies) {
		...
		for (int i = 0; i < serverPropertyListVector.size(); i++) {
			if ((CIMProperty)serverPropertyListVector.get(i) != null && 
					Linux_DHCPPoolsForEntity.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyListVector.get(i)).getName())) {
	...
	*/	
	public static boolean testClientServerIntegrity(CIMClient cimClient, Vector missedProperties, Vector unrecognizedProperties) {
		...
		for (int i = 0; i < serverPropertyList.size(); i++) {
			if ((CIMProperty)serverPropertyList.get(i) == null ||
					!Linux_DHCPPoolsForEntity.CIM_PropertyNameList.contains(((CIMProperty)serverPropertyList.get(i)).getName())) {
		...
		for (int i = 0; i < Linux_DHCPPoolsForEntity.CIM_PropertyNameList.size(); i++) {
				if (!serverPropertyNameList.contains(Linux_DHCPPoolsForEntity.CIM_PropertyNameList.get(i))) {
				unrecognizedProperties.add(Linux_DHCPPoolsForEntity.CIM_PropertyNameList.get(i));
				result = false;
			}
		}
		return result;
	...
	public static boolean isValid_Linux_DHCPPool(Linux_DHCPPool Linux_DHCPPool) {
		return true;
	}
