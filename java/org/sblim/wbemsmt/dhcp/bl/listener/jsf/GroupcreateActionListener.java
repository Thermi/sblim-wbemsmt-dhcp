...
public class GroupcreateActionListener extends TaskLauncherContextMenuEventListenerImpl {
	...
	public String processEvent(TaskLauncherTreeNodeEvent event) throws org.sblim.wbemsmt.exception.WbemSmtException {
		...
		TaskLauncherTreeNodeSelectorForCreate selector = new org.sblim.wbemsmt.dhcp.bl.listener.GroupcreateActionListenerSelector();
		selector.select(treeNode,adapter,"GroupcreateAction");

		if (selector.execute())
		{
			adapter = (org.sblim.wbemsmt.dhcp.bl.adapter.DhcpCimAdapter)selector.getAdapter();
		
			org.sblim.wbemsmt.dhcp.bl.container.wizard.jsf.NewGroupWizard wizard = new org.sblim.wbemsmt.dhcp.bl.container.wizard.jsf.NewGroupWizard(adapter);
