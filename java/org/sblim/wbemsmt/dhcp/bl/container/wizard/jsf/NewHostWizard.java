...
public class NewHostWizard extends JSFWizardBase implements IPageWizardAdapter {

	private org.sblim.wbemsmt.dhcp.wizard.NewHostWizardContainer container = null;
	
	String binding = "objectActionController.currentWizard.currentPanel.";
	
		org.sblim.wbemsmt.dhcp.bl.container.wizard.jsf.NewHostContainerImpl panel1 = null;
		org.sblim.wbemsmt.dhcp.bl.container.wizard.jsf.NewHostSummaryContainerImpl panel2 = null;
		
	final org.sblim.wbemsmt.dhcp.bl.adapter.DhcpCimAdapter adapter;
	
	public NewHostWizard(final org.sblim.wbemsmt.dhcp.bl.adapter.DhcpCimAdapter adapter) {
		super(adapter,ResourceBundleManager.getResourceBundle(new String[]{"messages","messagesDhcp"},LocaleManager.getCurrent(FacesContext.getCurrentInstance()).getCurrentLocale()),"NewHostWizard.title");
		this.adapter = adapter;
		this.container = new org.sblim.wbemsmt.dhcp.wizard.NewHostWizardContainer (adapter,new org.sblim.wbemsmt.dhcp.wizard.NewHostWizardContainerPanels()
 			{
            						//update the child objects
    				
					public Object getNewHost() throws WbemSmtException
					{
						...
						try
						{
    						panel1= new org.sblim.wbemsmt.dhcp.bl.container.wizard.jsf.NewHostContainerImpl(adapter,binding);
						...
						{
							throw new WbemSmtException("Cannot process Wizard NewHostWizard",e);
						}
    				}
            						//update the child objects
    				
					public Object getNewHostSummary() throws WbemSmtException
					{
						//Workaround for Bug https://issues.apache.org/jira/browse/MYFACES-1438
						//Create the panel everytime the panel is needed 
						//if (panel2!= null)
						...
						try
						{
    						panel2= new org.sblim.wbemsmt.dhcp.bl.container.wizard.jsf.NewHostSummaryContainerImpl(adapter,binding);
   ...
   //Workaround for a bug within myFaces - Everytime a new panel is created the childs are created also
	...
	public void countAndCreateChilds(DataContainer dataContainer) throws UpdateControlsException {
            						if (dataContainer instanceof org.sblim.wbemsmt.dhcp.bl.container.wizard.jsf.NewHostContainerImpl)
					{
        				    					//update the child objects
        										try
						{
						...
						{
							throw new UpdateControlsException("Cannot process Wizard NewHostWizard",e);
						}
    				}
            						if (dataContainer instanceof org.sblim.wbemsmt.dhcp.bl.container.wizard.jsf.NewHostSummaryContainerImpl)
					{
        				    					//update the child objects
        										try
						{
	...
	{
		return bundle.getString("NewHostWizard.finishText",bundle.getString("wizard.finishText"));
	}
	
	public String getJavaScriptWaitStatement()
	{
		String text = bundle.getString("NewHostWizard.createWaitText",bundle.getString("wizard.createWaitText"));
