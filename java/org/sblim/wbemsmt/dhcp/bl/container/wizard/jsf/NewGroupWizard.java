...
public class NewGroupWizard extends JSFWizardBase implements IPageWizardAdapter {
	...
	public NewGroupWizard(final org.sblim.wbemsmt.dhcp.bl.adapter.DhcpCimAdapter adapter) {
		super(adapter,ResourceBundleManager.getResourceBundle(new String[]{"messages","messagesDhcp"},LocaleManager.getCurrent(FacesContext.getCurrentInstance()).getCurrentLocale()),"NewGroupWizard.title");
		this.adapter = adapter;
		this.container = new org.sblim.wbemsmt.dhcp.wizard.NewGroupWizardContainer (adapter,new org.sblim.wbemsmt.dhcp.wizard.NewGroupWizardContainerPanels()
 			{
					...
					public Object getNewGroup() throws WbemSmtException
					{
						...
						{
							throw new WbemSmtException("Cannot process Wizard NewGroupWizard",e);
						}
    				}
            	 			});
		super.initialize();
   ...
   //Workaround for a bug within myFaces - Everytime a new panel is created the childs are created also
	...
	public void countAndCreateChilds(DataContainer dataContainer) throws UpdateControlsException {
            						if (dataContainer instanceof org.sblim.wbemsmt.dhcp.bl.container.wizard.jsf.NewGroupContainerImpl)
					{
						...
						{
							throw new UpdateControlsException("Cannot process Wizard NewGroupWizard",e);
						}
    				}
            		}   
	...
	{
		String text = bundle.getString("NewGroupWizard.createWaitText",bundle.getString("wizard.createWaitText"));
